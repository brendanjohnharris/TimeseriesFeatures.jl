...
function MultivariateFeature(
        method::Function, name = Symbol(method),
        keywords::Vector{String} = [""], description::String = ""
    )
    return Feature(; method, name, keywords, description)
end
function MultivariateFeature(
        method::Function, name, description::String,
        keywords::Vector{String} = [""]
    )
    return Feature(; method, name, keywords, description)
end
...
function (𝑓::AbstractMultivariateFeature)(X::DimensionalData.AbstractDimMatrix)
    return DimArray(𝑓(X.data), (dims(X, 2), dims(X, 2)))
end

const PairwiseOrMultivariate = Union{
    <:AbstractMultivariateFeature,
...
function (𝒇::MultivariateFeatureSet)(x::AbstractMatrix)
    return DimArray(
        permutedims(
            (cat(FeatureVector([𝑓(x) for 𝑓 in 𝒇], 𝒇)...; dims = ndims(x) + 1)),
            [ndims(x) + 1, (1:ndims(x))...]
        ),
        (
...
function (𝒇::MultivariateFeatureSet)(x::DimensionalData.AbstractDimMatrix)
    return DimArray(
        permutedims(
            (
                cat(
                    FeatureVector([𝑓(x |> collect) for 𝑓 in 𝒇], 𝒇)...;
                    dims = ndims(x) + 1
                )
            ), [ndims(x) + 1, (1:ndims(x))...]
        ),
        (_featuredim(getnames(𝒇)), dims(x, 2), dims(x, 2))
    ) |> FeatureArray
end
...
#     (U * S * S' * U') / (size(X, 1) - 1)
# end
Covariance_svd = MultivariateFeature(
    X -> cov(X), :Covariance_svd, "Sample covariance",
    ["covariance"]
)
Pearson_svd = MultivariateFeature(
    X -> cor(X), :Pearson_svd,
