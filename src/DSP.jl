_needsdsp() = error("DSP.jl must be loaded to use this feature; run `using DSP`")

# Filled by DSPExt; the fallbacks are less specific, so the extension's methods win
maybe_hilbert(::Any...) = _needsdsp()
maybe_welch_pgram(::Any...; kwargs...) = _needsdsp()

Analytic_Signal = Feature(
    maybe_hilbert, :Analytic_Signal,
    "Analytic signal of the time series, from the Hilbert Transform",
    ["transform", "phase", "amplitude", "hilbert"]
)
Analytic_Phase = Feature(
    x -> x |> maybe_hilbert .|> angle, :Analytic_Phase,
    "Analytic phase of the time series, from the Hilbert Transform",
    ["transform", "phase", "hilbert"]
)
Analytic_Amplitude = Feature(
    x -> x |> maybe_hilbert .|> abs, :Analytic_Amplitude,
    "Analytic amplitude of the time series, from the Hilbert Transform",
    ["transform", "amplitude"]
)
export Analytic_Signal, Analytic_Phase, Analytic_Amplitude
...
export pairwisephaseconsistency, phaselockingvalue, PPC, PPC_Analytic_Phase, PLV,
    PLV_Analytic_Phase
...
Implemented by `DSPExt` (loaded with `using DSP`).
"""
function bandpower end
bandpower(::Any...) = _needsdsp()

_numname(x) = x isa AbstractFloat && isinteger(x) ? Integer(x) : x # 1000 and 1000.0 alike

"""
    BandPower(edges; fs = 1, name = Symbol(:BandPower_, fs), psd)

A `SuperFeatureSet` giving the power in each of the `length(edges) - 1` successive
frequency bands `[edges[i], edges[i + 1])`. Requires DSP.jl.
Frequencies are in the same units as `fs`, which defaults to 1 (so that bands are given in
cycles per sample). The power spectral density is estimated by `psd`, a Welch periodogram
with a Hann window by default, and shared across all bands, so it is computed only once per
time series.
