...
function FeatureArray(
    ...
    return FeatureArray(data, format(dims, data), refdims, name, metadata)
end
function FeatureArray(
        data, dims::Tuple;
        refdims = (), name = NoName(),
...
function FeatureArray(
        data::AbstractArray, features::Union{Tuple{Symbol}, Vector{Symbol}};
        kwargs...
    )
    return FeatureArray(
        data,
...
function FeatureArray(
        data::AbstractVector, features::Union{Tuple{Symbol}, Vector{Symbol}},
...
function FeatureArray(
    ...
    return FeatureArray(
        data, (_featuredim(features), Dim{:timeseries}(timeseries));
        kwargs...
    )
end
function FeatureArray(
    ...
    return FeatureArray(
        ...
        (
            _featuredim(features),
            [Dim{x.first}(x.second[:]) for x in otherdims]...,
        ); kwargs...
    )
end
...
@inline function rebuild(
        A::AbstractFeatureArray, data::AbstractArray, dims::Tuple,
        refdims::Tuple, name, metadata
    )
    if dims isa Tuple
        dims = replace(collect(dims), nothing => AnonDim())
    end
    return FeatureArray(data, Tuple(dims); refdims, name, metadata)
end
...
function getindex(A::T, 𝑓::FeatureSetUnion)::T where {T <: AbstractFeatureVector}
    return getindex(A, fidx(𝑓))
end

function getindex(A::AbstractFeatureArray, 𝑓::FeatureUnion, i, I...)
    return getindex(A, fidx(𝑓), i, I...)
end
function getindex(
        A::AbstractFeatureArray{T, 2, D}, 𝑓::Symbol, i,
        I...
    ) where {T, D <: Tuple{FeatDim, AnonDim}}
    return parent(getindex(A, fidx(𝑓), i, I...))
end
function setindex!(A::AbstractFeatureArray, x, 𝑓::FeatureUnion, i, I...)
    return setindex!(A, x, fidx(𝑓), i, I...)
end
...
function getindex(A::AbstractFeatureArray, 𝑓::FeatureUnion)
    return getindex(A, 𝑓, fill(:, ndims(A) - 1)...)
end
function setindex!(A::AbstractFeatureArray, x, 𝑓::FeatureUnion)
    return setindex!(A, x, 𝑓, fill(:, ndims(A) - 1)...)
end
...
function FeatureArray(X::AbstractArray, 𝒇::AbstractFeatureSet, args...; kwargs...)
    return FeatureArray(X::AbstractArray, getnames(𝒇), args...; kwargs...)
end

function (FeatureArray{T, N} where {T})(
        x::AbstractArray{S, N}, args...;
        kwargs...
    ) where {S, N}
    return FeatureArray(x, args...; kwargs...)
end
...
function LabelledFeatureArray(x::AbstractArray, F::AbstractDimArray, f; kwargs...)
    return FeatureArray(
        parent(F), f; # Don't have a DimArray as data
        name = _name(x),
        metadata = DimensionalData.metadata(x),
        refdims = DimensionalData.refdims(x), kwargs...
    )
...
function LabelledFeatureArray(
        x::AbstractDimArray{<:AbstractArray}, F::AbstractArray, f;
        kwargs...
    )
...
function (𝒇::FeatureSet)(
        x::AbstractVector{<:T},
        return_type::Type = Float64
    ) where {T <: Number}
    y = [𝑓(x) for 𝑓 in 𝒇]
    y = convert(Vector{return_type}, y)
    return y = LabelledFeatureArray(x, y, 𝒇)
end
function (𝒇::FeatureSet)(
        X::AbstractArray{<:AbstractVector}, return_type::Type = Float64;
        chart = Chart()
    )
...
function (𝒇::FeatureSet)(
        X::AbstractArray{T, N}, return_type::Type = Float64;
        kwargs...
    ) where {T <: Number, N}
